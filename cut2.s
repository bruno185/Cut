...
; modification 2022
; pour éviter erreur E/S si on ne lance la CUT depuis S6D1.
; Le principe : la dernière commande est à la adresse $280
; Exemple /MONDISK/CUT (puisqu'on vient de lancer CUT)
; on extrait le path de cette chaine en parcourant la chaine
; et en s'arretant au 2ème / 
          ldy #$00      ; y : compteur de car. "/"
          ldx #$00
setpath
          lda $280,x    ; copie la string 
          sta path,x    ; vers le param de setprefix (mli)
          inx           ; car. suivant
          cmp #$2F      ; "/" ?
          bne setpath   ; non : loop
          iny           ; oui : compteur++
          cpy #2        ; 2 "/" ?
          bne setpath   ; non : loop
          dex           ; oui, on ajuste la longueur
          stx path      ; ==> 1er car. du buffer path.
